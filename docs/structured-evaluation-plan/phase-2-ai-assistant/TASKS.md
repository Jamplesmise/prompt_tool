...
- [x] **5.2.1** 设计 System Prompt ✅
  - 精简输出格式
  - 明确字段说明
  - 提供示例

- [x] **5.2.2** 测试 Prompt 效果 ✅
  - 多场景测试
  - 调整优化
...
- [x] **5.4.2** 实现 Step 1: 选择模型 ✅
  - 模型下拉选择器
  - 已配置模型列表展示
  - 使用提示

- [x] **5.4.3** 实现 Step 2: 描述场景 ✅
  - 场景名称输入
  - 多行文本描述区域
  - 引导提示（输入/输出/关键字段）
  - "生成配置"按钮
...
- [x] AI 配置助手可正常生成 Schema ✅
- [x] 生成的 Schema 结构正确、可用 ✅
- [x] 可选择任意已配置模型进行生成 ✅
- [x] 生成后可直接下载数据集模板 ✅
- [x] 条件表达式正确求值 ✅
- [x] 条件不满足时字段被跳过 ✅
- [x] critical_first 聚合正确 ✅
- [x] custom 聚合表达式正确执行 ✅
- [x] 结果详情展示字段级评估 ✅
- [x] 聚合计算明细展示正确 ✅
- [x] 字段级统计数据准确 ✅
...
| 2025-12-11 | 8.3-8.6 UI 与测试完善 | 条件配置/聚合 UI + 任务流程改造 + 集成测试 |
...
**主要交付物**:
1. **AI 配置助手**: aiSchemaGenerator.ts + schemaAssembler.ts + 4步骤向导页面
2. **条件表达式求值器**: conditionEvaluator.ts，支持安全沙箱执行
3. **聚合策略升级**: critical_first + custom 自定义表达式模式
4. **字段级 API**: 结果详情 API + 字段统计 API
5. **前端升级**: ResultDetailV2 + FieldAnalysisPanel

**单元测试**: 18+ tests (conditionEvaluator)
