...
import { useInputSchema, useUpdateInputSchema } from '@/hooks/useSchemas'
...
export default function EditInputSchemaPage() {
  const router = useRouter()
  const params = useParams()
  const id = params.id as string

  const [form] = Form.useForm<FormValues>()
  const { data: schema, isLoading } = useInputSchema(id)
  const updateMutation = useUpdateInputSchema()

  // 变量列表状态
  const [variables, setVariables] = useState<InputVariableDefinition[]>([])
  ...
  useEffect(() => {
    if (schema) {
      ...
      setVariables(schema.variables.length > 0 ? schema.variables : [createEmptyVariable()])
    }
  }, [schema, form])
