...
import { useOutputSchema, useUpdateOutputSchema } from '@/hooks/useSchemas'
...
export default function EditOutputSchemaPage() {
  const router = useRouter()
  const params = useParams()
  const id = params.id as string

  const [form] = Form.useForm<FormValues>()
  const { data: schema, isLoading } = useOutputSchema(id)
  const updateMutation = useUpdateOutputSchema()
  const { data: evaluators } = useEvaluators()

  // 字段列表状态
  const [fields, setFields] = useState<OutputFieldDefinition[]>([])
  ...
  useEffect(() => {
    if (schema) {
      ...
      setFields(schema.fields.length > 0 ? schema.fields : [createEmptyField()])
    }
  }, [schema, form])
